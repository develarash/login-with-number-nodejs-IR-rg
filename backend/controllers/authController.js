...
const handleSmsCode = catchFn(async (req, res, next) => {
  ...
  if (!phoneNumber)
    return next(new AppError("شماره موبایل نمیتواند خالی بماند", 400));
  ...
  client
    .manualSendCode(phoneNumber, `${verificationCode}کد تایید شما \n   \n `)
    .then(async (messageId) => {
      ...
      if (!user) {
        ...
        // console.log(uniqueString().slice(3,10));
      }
      ...
      return res.status(200).json({
        message: "کد تایید را به شماره ی مورد نظر ارسال کردید . ",
      });
    });
});
...
const handleSmsAuth = catchFn(async (req, res, next) => {
  ...
  if (user.verificationCode !== verificationCode)
    return next(new AppError("کد تایید صحیح نیست", 400));
  ...
  // همه کوکی ها به جز کوکی که برگردوندیم
